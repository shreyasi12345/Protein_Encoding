...
def main():
    ...
    if not read_fasta(input, names, seqs):
    ...
    if encoding == "SSEB" or encoding == "SSEC" or encoding == "SSPB" or encoding == "SSPAC":
        n = 10
        path = getArgument(argv, argv + argc, "--path")
        if path == "":
            print("Error: A path where .ss2, .spd33 or .spXout files are located is needed. Use the --path <path> argument.")
            return 0
        generate = argumentExists(argv, argv + argc, "--generate")
        pathExists = directoryExists(path)
        if pathExists == -1:
            return 0
        ...
        headers.append("name")
        if encoding == "SSEB":
            seqLength = checkFastaSameLength(seqs)
            if seqLength == -1:
                crop = argumentExists(argv, argv + argc, "--crop")
                if crop:
                    seqLength = getMinSequenceLength(names, seqs)
                    cropSequences(seqs, seqLength)
                else:
                    print("Error: All sequences must have the same length. You can use the --crop argument, so it crops the sequences automatically.")
                    return 0
            ...
            n = 1
            nS = getArgument(argv, argv + argc, "--n")
            if nS != "":
                if not isNumber(nS):
                ...
                if n < 1:
                    print("Error: The n value must be a number greater or equal than 1.")
                    return 0
            minLength = getMinSequenceLength(names, seqs)
            if minLength < n + 1:
        ...
        for i in range(len(seqs)):
            ...
            if not file1 and not file2 and not file3:
                print("No predicted secondary structure files (.ss2, .spd33 or spXout) found for sequence " + names[i] + ".")
                ...
                print("Secondary structure files (.ss2, .spd33 or spXout) for sequence " + names[i] + " could not be opened.")
            else:
                encodedProtein = []
                if encoding == "SSEB":
                    if file1.is_open():
